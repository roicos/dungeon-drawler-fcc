  ...
  createBoard(width, height, roomsNum){ // 0-wall, 1-dungeon
    ...
    function checkRoom(room){
      for(var y=room.begin.row-2; y<=room.end.row+2; y++){
          for(var x=room.begin.col-2; x<=room.end.col+2; x++){
            check = (board[y] != undefined && board[y][x]);
            if(!check){
              return false;
            }
          }
    ...
    for(var num=0; num<roomsNum; num++){
      ...
      while(check != true){
        ...
        var room = {"begin" : {"row" : row-1, "col":col-1}, "end" : {"row" : row+1, "col":col+1}, "completed" : false};
    ...
    // dig rooms
    while (!areRoomsCompleted){
      for(var i=0; i<rooms.length; i++){
        if(!rooms[i].completed && checkRoom(rooms[i])){
